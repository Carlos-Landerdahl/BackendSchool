...
public class FilialDao implements IDao<FilialHotel> {
    ...
    public FilialHotel buscarFilial(int id) throws SQLException {
        try (Connection connection = configuracaoJDBC.conectarBancoDeDados();
             PreparedStatement ps = connection.prepareStatement("SELECT * FROM Filiais WHERE Id = ?")) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return new FilialHotel(
                            rs.getInt("Id"),
                            rs.getString("Nome_da_filial"),
                            rs.getString("Rua"),
                            rs.getInt("Numero"),
                            rs.getString("Cidade"),
                            rs.getString("Estado"),
                            rs.getBoolean("E_5_estrelas")
                    );
                } else {
                    return null;
                }
